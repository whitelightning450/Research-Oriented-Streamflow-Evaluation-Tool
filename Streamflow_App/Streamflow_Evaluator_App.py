...
class LULC_Eval():
    ...
    def __init__(self, CLIENT_ID, CLIENT_SECRET, ACCESS_TOKEN, model ,state,  startDT, endDT):
        self = self
    ...
    def get_NWIS(self):
        ...
        #Adjust this line to grant access to data to others
        boxfile = bf(CLIENT_ID=self.CLIENT_ID, CLIENT_SECRET=self.CLIENT_SECRET, ACCESS_TOKEN=self.ACCESS_TOKEN)
        ...
        U_state = self.state.upper()
        self.NWIS_sites = Streamstats[Streamstats['state_id'] == U_state]
        ...
        self.NWIS_sites.reset_index(inplace = True)
    ...
    #Get model data via threading function
    def BoxSDK_Mod_Theading(self, boxfile, site_id, reach, format):
        try:
            #NHD_reach = str(df.NHD_reachid[reach])
            reach_id = 'NWM_'+ str(reach)
            csvFile = boxfile.GetFile(str(site_id[reach_id])).decode("utf-8")
            ioFile = io.StringIO(csvFile)
            ...
            NHD_reach_id = reach_id[4:]
            #print(NHD_reach_id)
            self.Mod_data[NHD_reach_id] = Mod_flow[cols[0]]
            
        except:
            print('Site: ', reach, ' not loaded, will run again')
            #remove item from list

    def BoxSDK_NWIS_Theading(self,boxfile, site_id, reach, format):
        self.site_id = site_id
        try:
            reach_id = 'NWIS_'+reach
            NWIS_reach = str(self.site_id[reach_id])
            #print(reach_id, NWIS_reach)
            #reach_id = 'NWM_'+ NHD_reach
            csvFile = boxfile.GetFile(NWIS_reach).decode("utf-8")
            ioFile = io.StringIO(csvFile)
            NWIS_flow = pd.read_csv(filepath_or_buffer = ioFile, delimiter = ",")
            NWIS_flow['time'] ='12:00:00'
            NWIS_flow['Datetime'] = pd.to_datetime(NWIS_flow['Datetime']+ ' ' + NWIS_flow['time'], format = format)
            #print(NWIS_flow)
            NWIS_flow.set_index('Datetime', inplace = True)
            NWIS_flow = NWIS_flow.loc[self.startDT:self.endDT]
            NWIS_flow.pop('Unnamed: 0')
            reach_id = str(reach_id[5:])
            #display(NWIS_flow['USGS_flow'])
            #print(reach_id)
            self.NWIS_data[reach_id] = NWIS_flow['USGS_flow']
            #print(reach, reach_id)

        except:
            print('Site: ', reach, ' did not load, retrying')
        
        
    def Threader(self,boxfile, Thred_func, df, site_id, format, dfreach):
        threads = []
        pbar = ProgressBar()
        reaches = df[dfreach]
        for reach in pbar(reaches):    

            #make threading function
            t = threading.Thread(target=Thred_func, args=(boxfile, site_id, reach, format))
    ...
    def Get_BoxSDK_Mod_data_Threaded(self, df):
        print('Loading in ',self.model, ' predictions from ', self.startDT, ' to ', self.endDT)
        models = [self.model]
        model_dict ={}
        #Load in dataframe connecting the model nhd predictions
        #Get the Model prediction folder
        for model in models:
            results = self.client.search().query(query = model,type = 'folder', limit = 1)
        ...
        site_dict = {}
        folder_id =  model_dict[self.model]
        folder = self.client.folder(folder_id)
        site_key = self.model + '_site_keys.csv'
        results = self.client.search().query(query = site_key, limit=1, file_extensions=['csv'], ancestor_folders=[folder])

        for result in results:
            name = result.name
            if name == site_key:
                name = result.name[:-4]
                site_dict[name] = result.id
                #print(f'The item ID is {result.id} and the item name is {name}')
                break
        #print(site_dict)
        #open file of site ids
        boxfile = bf(CLIENT_ID=self.CLIENT_ID, 
                     CLIENT_SECRET=self.CLIENT_SECRET, 
                     ACCESS_TOKEN=self.ACCESS_TOKEN)
        csvFile = boxfile.GetFile(site_dict[site_key[:-4]]).decode("utf-8")
        ioFile = io.StringIO(csvFile)
        site_id = pd.read_csv(filepath_or_buffer = ioFile, delimiter = ",")
        site_id.pop('Unnamed: 0')

        #Get Keyword list of comparision reaches
        NHD_list = []
        for NHD in self.comparison_reaches:
            #this will need to change to NHD
            name = 'NWM_'+ str(NHD)
            NHD_list.append(name)
        #note - might have to change the NWM in site_id hardcode
        mask = site_id.NWM.apply(lambda x: any(item for item in NHD_list if item in x))
        site_id = site_id[mask]
        site_id.reset_index(inplace= True, drop = True)

        #Turn into dictionary to loop through
        cols = site_id.columns
        site_id.set_index(cols[0], inplace = True)
        site_id = site_id.T
        site_id = site_id.to_dict('records')[0]
        self.site_id = site_id

        print('There are ', len(df), ' collocated NWIS/NHD reaches in ', self.category, ': ', self.size)

        #load NHD model data into dictionary,
        #I think we can make a function to multithread this to speed it up
        if len(self.site_id) >= 1:
            #threads = []
            print('API connection successful, loading.')
            format = '%Y-%m-%d %H:%M:%S'
            #pbar = ProgressBar()
            #for reach in pbar(np.arange(0,len(df),1)):    
                #need to run the function twice to get all items bc of threader issues
            for run in np.arange(0,2,1):
                self.Threader(boxfile, self.BoxSDK_Mod_Theading, df,self.site_id, format, 'NHD_reachid')

        else:
            print('API connection was not successful, make sure the credentials are correct.')
            
            
    def Get_BoxSDK_NWIS_data_Threaded(self,df):
        print('Loading in NWIS predictions from ', self.startDT, ' to ', self.endDT)


        #load the NWIS site key file : 1184330851812
        #find NWIS site file id's of interest
        #load and format NWIS site data for start-end dates of interest

        NWIS_site_key_file_id = '1184330851812'
        #open file of site ids
        boxfile = bf(CLIENT_ID=self.CLIENT_ID, 
                     CLIENT_SECRET=self.CLIENT_SECRET, 
                     ACCESS_TOKEN=self.ACCESS_TOKEN)
        csvFile = boxfile.GetFile(NWIS_site_key_file_id).decode("utf-8")
        ioFile = io.StringIO(csvFile)
        site_id = pd.read_csv(filepath_or_buffer = ioFile, delimiter = ",")
        site_id.pop('Unnamed: 0')

        #Get Keyword list of reaches: self.NWIS_sites

        mask = site_id.NWIS.apply(lambda x: any(item for item in df.NWIS_site_id if item in x))
        site_id = site_id[mask]
        site_id.reset_index(inplace= True, drop = True)
        ...
        if len(self.site_id) >= 1:
            ...
            #for reach in pbar(np.arange(0,len(df),1)):    
                #need to run the function twice to get all items bc of threader issues
            for run in np.arange(0,2,1):
                self.Threader(boxfile, self.BoxSDK_NWIS_Theading, df,site_id, format, 'NWIS_site_id')
        else:
            print('API connection was not successful, make sure the credentials are correct.')
    ...
    def prepare_comparison(self, df):
        ...
        #self.NWIS_data = pd.DataFrame(columns = self.NWIS_sites)
        self.NWIS_data = pd.DataFrame()
        self.Mod_data = pd.DataFrame()
        
        #Get model data
        self.Get_BoxSDK_Mod_data_Threaded(df)
       
       #need to add an error item here to go get data that did not make it into the DF, 
        #look into multithread to see how to make sure it all gets there, 
        #look into snow model, may have done this already.
        cols = list(map(str, list(df.NHD_reachid)))
        self.Mod_data = self.Mod_data[cols]
        self.NWIS_data['Datetime'] = self.Mod_data.index
        self.NWIS_data.set_index('Datetime', inplace = True, drop =True)

        #Get NWIS data
        print('Getting NWIS data')
        self.Get_BoxSDK_NWIS_data_Threaded(df)
    ...
    def Map_Plot_Eval(self, freq, df, size, supply):
        ...
        for i in np.arange(0, len(self.df_map),1):
            ...
            reach = str(self.df_map['NHD_reachid'][i])
